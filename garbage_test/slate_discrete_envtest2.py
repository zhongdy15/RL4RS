...
config = {"epoch": 10000, "maxlen": 64, "batch_size": 2, "action_size": 284,
...
for modified_step in range(config["max_steps"]):
    print("###############################################################################")
    print("###############################################################################")
    print("modified step: ", modified_step)
    print("###############################################################################")
    print("###############################################################################")

    offline_list = []
    for test_value in range(284):
        obs = env.reset()
        # print('batchsize of batched environment: ', len(obs))

        for i in range(config["max_steps"]):
            action = env.offline_action
            # print('action: ', action[0], "available actions: ", obs[0]["action_mask"][action[0]])

            if i == modified_step:
                original_action = action[0]
                modified_action = test_value
                action[0] = test_value
                offline_list.append(original_action)

            next_obs, reward, done, info = env.step(action)

            if done[0]:
                print('step: ', modified_step, ' action', test_value, ' reward: ', reward[0], 'offline action: ', original_action, 'offline reward: ', env.offline_reward[0],
                      ' done: ', done[0])
                break
    ...
    print("MAX: " + str(max(offline_list)) + " MIN: " + str(min(offline_list)))
    max_list.append(max(offline_list))
    min_list.append(min(offline_list))
